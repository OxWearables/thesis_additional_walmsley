...
for (i in 1:4) {
  ...
  HRs <- calc_HRs(model = cox_mod_simple, row = "piv1", ilr_diff = ilr_diff)
  ev_nums <- get_ev_nums(cox_mod_simple)
  rec_dat[(i - 1) * 5 + 1,] <- c(comp_name, diff, "unweighted", HRs, ev_nums)
  rm(HRs, ev_nums)
  ...
  HRs <- calc_HRs(model = cox_mod_weighted, row = "piv1", ilr_diff = ilr_diff)
  ev_nums <- get_ev_nums(cox_mod_weighted)
  rec_dat[(i - 1) * 5 + 2,] <- c(comp_name, diff, "weighted", HRs, ev_nums)
  rm(HRs, ev_nums)
  ...
  rec_dat[(i - 1) * 5 +3,] <- c(comp_name, diff, "simple_weights_trim", HRs, ev_nums)
  rm(HRs, ev_nums)
  ...
  rec_dat[(i - 1) * 5 + 4,] <- c(comp_name, diff, "weighted - alt syntax", HRs, ev_nums)
  rm(HRs, ev_nums)
  ...
  rec_dat[(i - 1) * 5 +5,] <- c(comp_name, diff, "simple_weights", HRs, ev_nums)
  rm(HRs, ev_nums)
}
```
Inspect results: 
