...
# Check ordering of factors has worked (needed for imputation)
for (var in var_list_imp){
  print(var)
  print(class(df_all[, var]))
  if ("factor" %in% class(df_all[, var])){
    print(is.ordered(df_all[, var]))
  }
} 
...
for (i in 1:5){
  ...
  cat("max VIF:", max(car::vif(loc_glm))) ## variance inflation
  # The large VIFs are to do with the two variables 
...
An even starker example: look at pa fail to adjust for sex (or anything else in fact)
```{r}
cox_mod_rcheck1 <- coxph(Surv(age_entry, age_exit, CVD_event) ~ BMI_cats, weights = df_in$inverse_wt_stab_trim,
    data = df_in, id = eid, robust = TRUE )
  
cox_mod_rcheck2 <- coxph(Surv(age_entry, age_exit, CVD_event) ~ BMI_cats, data = df_in)
