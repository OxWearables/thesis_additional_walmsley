...
# Note this is only needed if we want to grab coefficients from this to present later... 

# THIS IS WHERE WE NEED MICE
# mod_confint_log_odds <- confint(log_inc)
# mod_confint <- as.data.frame(exp(mod_confint_log_odds))
# mod_coef <- as.data.frame(exp(coefficients(log_inc)))
# tab_wt_model <- cbind(mod_coef, mod_confint)
# tab_wt_model_f <-
#   format(round(tab_wt_model, digits = 2), nsmall = 2)
# tab_wt_model_new <- data.frame(ncol)
# t <-
#   knitr::kable(
#     tab_wt_model_f,
#     format = "latex",
