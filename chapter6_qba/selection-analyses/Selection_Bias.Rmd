...
df_all$Category_Self_reported_MPA <- cut(df_all$Self_reported_MPA, quantile(df_all$Self_reported_MPA, seq(0, 1, 0.2), na.rm = TRUE))
df_all$Category_Self_reported_VPA <- cut(df_all$Self_reported_VPA, c(-1, 0.000000000000001, 10.000000000000000001, 20.00000000000001, 30.0000000000000001, 60.00000000000001, 1441))
...
var_list_imp <-
  c(
    ...
    "poor_health",
    "meds",
    "Category_Self_reported_MPA",
    "Category_Self_reported_VPA",
    "Job_involves_walking_standing",
    "Job_involves_activity",
    "Self_reported_usual_walking_pace",
...
writeRDS(imp, "imputed_dataset.RDS")
print(Sys.time())
```
  ...
  with(
    ...
    glm(
      in_analytic_sample ~ baseline_age_ten_years + sex + ethnicity + smoking +
        ...
        Category_Self_reported_MPA + Category_Self_reported_VPA + Job_involves_walking_standing +
        Job_involves_activity + Self_reported_usual_walking_pace + Systolic_blood_pressure +
        log_HbA1c + log_C_Reactive_Protein + LDL_cholesterol + HDL_cholesterol +
        log_Triglycerides,
      family = binomial
    )
  )
...
for (i in 1:6){
  ...
  loc_glm <-glm(
    in_analytic_sample ~ baseline_age_ten_years + sex + ethnicity + smoking +
        alcohol + fruit_and_veg_cats + red_and_processed_meat_cats + oily_fish +
        TownsendDeprIndexRecruit + education_cats + log_BMI + poor_health + meds +
        log_1plus_Self_reported_MPA + log_1plus_Self_reported_VPA + Job_involves_walking_standing +
...
An even starker example: look at smoking and fail to adjust for sex (or anything else in fact)
```{r}
cox_mod_rcheck1 <- coxph(Surv(age_entry, age_exit, CVD_event) ~ smoking, weights = inverse_wt_stab_trim,
    data = df_in, id = eid, robust = TRUE )
  
cox_mod_rcheck2 <- coxph(Surv(age_entry, age_exit, CVD_event) ~ smoking, data = df_in)
