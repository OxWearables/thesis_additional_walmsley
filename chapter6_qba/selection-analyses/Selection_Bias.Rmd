...
This script uses data from three places: 

- Data produced by 'data-preprep-ipw.R' [in current folder]. This is data for the whole dataset processed following same procedure used on the data in earlier chapters.  
- Data from earlier chapters (defining the analytic sample).
- Additional data from the processing for Chapter 3 (the descriptive chapter), which contains a few additional columns (those in vars_sr_act and vars_biom_tab_only, mostly related to self-reported activity and biomarkers).
...
Setting up lists of variables of different types: 
```{r}
vars_orig <-
  c(
    "sex",
...
vars_sr_act <-   c(
  "Self_reported_MET_minutes_walking", # NOTE: I'm using the returned variables to define these. But I'm not sure how missing data is determined for these variables, as I find it tricky to post impute. DNK/ PNA/ Unable to walk seem to be sometimes treated as missing and sometimes as 0, but I can't work out the rules exactly. 
...
var_list_minus_age <-
  c(vars_orig, vars_health_orig, vars_sr_act, vars_biom_tab_only)
...
This was a little bit of code to process job-related variables, which are only present in people with a job.

Note: I believe this isn't actually now used anywhere. 
```{r}
emp_status <- rep(NA, times = nrow(df_other))

in_emp <- grepl("In paid employment or self-employed", df_other$employed_i0)
not_in_emp <- !in_emp &  grepl("Retired|Looking after home and/or family|Unemployed|Unable to work because of sickness or disability|Doing unpaid or voluntary work|Full or part-time student", df_other$employed_i0)

emp_status[in_emp] <-  "Employed"
emp_status[not_in_emp] <- "Not_employed"
...
for (var in vars_sr_act[4:5]) {
  ...
  # This seems to work as expected for NA variables but ideally would have careful NA treatment
}
```
...
```{r}
df_all <- merge(df_all,
                df_other[, c("eid", vars_sr_act, vars_biom_tab_only)],
                by = "eid", all.x = TRUE)
# We are left joining here because df_all has already had exclusion of people with prevalent CVD
```

Note we do have an interesting question about prevalent CVD, because there could be people who "shouldn't" be here because they either have CVD or die before the start of the accelerometry study i.e. they would never have actually been eligible to be in our eventual cohort. So in this sense we're making an assumption in the analysis. It's *probably* a conservative assumption as they're probably generally sicker. So further upweighting people with similar characteristics is probably generally reasonable. However, (*solution*) we'll also do a sensitivity analysis later for people who had had CVD or died before start of accelerometry (see below).
...
for (var in c("baseline_age", var_list_minus_age)) {
  ...
  # Relabel empty strings as missing----------------------------------------
  ...
  if (var %in% c(
    ...
    if (min(as.numeric(df_all[, var]), na.rm = TRUE) == 0) {
      df_all[, paste0("log_1plus_", var)] <-
        log(as.numeric(df_all[, var]) + 1)
    } else {
...
# Add age per ten years variable
...
} # Note that this should no longer be needed as I swapped out time based variables formatted as character for the corresponding MET minute variables (to automatically deal with the difficulties on scales of reporting). It's harmless so I've left it in for traceability. 

df_all$Category_METmin_walking <-
  cut(
    df_all$Self_reported_MET_minutes_walking,
    c(-1, 0.5, 500.5, 1000.5, 20000),
    labels = c("No walking", "1-500", "501-1000", "1001+")
  )
df_all$Category_METmin_MPA <-
  cut(
    df_all$Self_reported_MET_minutes_MPA,
    c(-1, 0.5, 500.5, 1000.5, 20000),
    labels = c("No moderate activity", "1-500", "501-1000", "1001+")
  )
df_all$Category_METmin_VPA <-
  cut(
    df_all$Self_reported_MET_minutes_VPA,
    c(-1, 0.5, 500.5, 1000.5, 20000),
    labels = c("No vigorous activity", "1-500", "501-1000", "1001+")
  )
```
Query: on returning to this the missingness patterns of derived IPAQ-based variables are very hard to explain.
...
- Note: I have got rid of job involves walking/standing variables as they are not that helpful and are causing a mess for imputation because of logical dependencies (they share a category of not being employed)
```{r}
...
imp$meth
```
Logistic regression model in whole dataset: 
```{r}
...
Do logistic regression model in each imputed dataset and return linear predictor of probability of inclusion for each participant:
```{r}
...
for (i in 1:5){
  ...
  print(car::vif(loc_glm))
  cat("max VIF:", max(car::vif(loc_glm)[, "GVIF"])) ## variance inflation

  ## Consider whether the logit as a linear function of the variable looks reasonable (commented out due to time taken to run)
  # num_vars <-   c( 
  #   "baseline_age_ten_years",
  #   "BMI",
  #   "Systolic_blood_pressure",
...
lp_av <- apply(lp_glm, 1, mean) # Take mean of linear predictors across all 5 imputations + models
p_av <- exp(lp_av)/(1+exp(lp_av)) # Invert logit transformation

df_all$selec_prob <- p_av
```

... There could be an issue if there is any scrambling of rows, but we should be able to see this from table. [Later note: I think this is referring to the possibility that the mice functions scramble rows in the dataset, which they don't,]

## Post processing of weights

Calculate Stabilised Trimmed Inverse Probability of Selection Weights: 

- Stabilisation shouldn't impact analytic results in model (shown later in this markdown), as these are invariant to multiplication by a constant. However, it will ensure weights are on a sensible, easy to read scale. I think it's more important when IPW used for confounding (?), but I don't know about this context.
- Trimming just ensures outlyingly high weights don't influence too much.
```{r}
selec_prob_all <-
  nrow(df_in) / nrow(df_all) # Look at overall probability of inclusion (everyone in df_all should be included in model due to imputation)
df_all$inverse_wt_stab <- selec_prob_all / df_all$selec_prob 
...
```{r}
df_in$pa <- df_in$acc.overall.avg # df_in$OveralAccelerAverag is maybe a better option here to truly match returned UKB data
df_in$age_entry_years <- as.double(df_in$age_entry)/365.25
df_in <- merge(df_in, 
               df_all[, c("eid", vars_biom_tab_only, vars_sr_act,  "Category_METmin_walking", 
    "Category_METmin_MPA",
    "Category_METmin_VPA", 
...
# Sensitivity analysis: redo everything in restricted cohort
...
tab_wt_model_f_rest <-
  cbind(mod_coef_rest[, 1, drop = FALSE], format(round(mod_coef_rest[, 2:4], digits = 2), nsmall = 2))
...
write(t_rest, "plots/table-weight-model-restricted-2dp.tex")
```
...
if (nrow(df_in)!= 87498){ # This is just checking that everyone in df_in is in df_res (i.e. the merge worked), which should be the case based on the conditions used to define the restricted dataset
...
## Concern - weighting never makes a difference
...
## Different ways of achieving same end

- unstabilised trimmed weights (expect identical)
- alternative syntax (expect identical)
- unstabilised untrimmed weights (expect similar)
- weights treated as case weights (expect similar but not necessarily that similar)
...
qtsimple <- quantile(df_in$inverse_wt_simple, 0.99)
df_in$inverse_wt_simple_trim <- ifelse(df_in$inverse_wt_simple > qtsimple, qtsimple, df_in$inverse_wt_simple)
```

Cox model:
```{r}
rec_dat <- make_rec_dat_frame(24)
...
for (i in 1:4) {
  ...
  HRs <- calc_HRs(model = cox_mod_simple, row = "piv1", ilr_diff = ilr_diff)
  ev_nums <- get_ev_nums(cox_mod_simple)
  rec_dat[(i - 1) * 6 + 1,] <- c(comp_name, diff, "unweighted", HRs, ev_nums)
  rm(HRs, ev_nums)
  ...
  HRs <- calc_HRs(model = cox_mod_weighted, row = "piv1", ilr_diff = ilr_diff)
  ev_nums <- get_ev_nums(cox_mod_weighted)
  rec_dat[(i - 1) * 6 + 2,] <- c(comp_name, diff, "weighted", HRs, ev_nums)
  rm(HRs, ev_nums)
  
      # Weighted Cox model, simple weights---------------------------------------- 
  cox_mod_simple_weights_trim <- coxph(
    Surv(age_entry, age_exit, CVD_event) ~
      strata(sex) + ethnicity + smoking + alcohol +
      fruit_and_veg_cats + oily_fish +
      red_and_processed_meat_cats + education_cats +
      TDI_quartiles +
      piv1 + piv2 + piv3, weights = inverse_wt_simple_trim,
    data = loc, id = eid, robust = TRUE # see notes on coxph function, robust = TRUE (i.e. robust SEs) needed if weights are sampling weights.
    # This is the most likely place for errors in this code 
    # But it seemes that coxph with weights and robust = TRUE treats weights as sampling weights
  )
  assign(paste0("cox_simple_weights_trim_", comp_name), cox_mod_simple_weights_trim)
  HRs <- calc_HRs(model = cox_mod_simple_weights_trim, row = "piv1", ilr_diff = ilr_diff)
  ev_nums <- get_ev_nums(cox_mod_simple_weights_trim)
  rec_dat[(i - 1) * 6 +3,] <- c(comp_name, diff, "simple_weights_trim", HRs, ev_nums)
  rm(HRs, ev_nums)
  ...
  rec_dat[(i - 1) * 6 + 4,] <- c(comp_name, diff, "weighted - alt syntax", HRs, ev_nums)
  rm(HRs, ev_nums)
  ...
  rec_dat[(i - 1) * 6 +5,] <- c(comp_name, diff, "simple_weights", HRs, ev_nums)
  rm(HRs, ev_nums)
  
  # Weighted Cox model, weights treated as case weights---------------------------------------- 
  # I'm not clear this has actually been achieved here, the function documentation is unclear 
  # See:  https://stats.stackexchange.com/questions/595082/how-do-the-defaults-for-the-weights-argument-and-the-robust-argument-work-in 
  cox_mod_case_weights <- coxph(
    Surv(age_entry, age_exit, CVD_event) ~
      strata(sex) + ethnicity + smoking + alcohol +
      fruit_and_veg_cats + oily_fish +
      red_and_processed_meat_cats + education_cats +
      TDI_quartiles +
      piv1 + piv2 + piv3, weights = inverse_wt_stab_trim,
    data = loc, robust = FALSE)
  assign(paste0("cox_case_weights_", comp_name), cox_mod_case_weights)
  HRs <- calc_HRs(model = cox_mod_case_weights, row = "piv1", ilr_diff = ilr_diff)
  ev_nums <- get_ev_nums(cox_mod_case_weights)
  rec_dat[(i - 1) * 6 +6,] <- c(comp_name, diff, "case_weights", HRs, ev_nums)
  rm(HRs, ev_nums)
}
```
Inspect results: 
