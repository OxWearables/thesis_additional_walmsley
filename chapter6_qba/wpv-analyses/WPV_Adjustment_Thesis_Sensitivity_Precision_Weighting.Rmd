    ...
    if (i == 0) {
      ...
      if (!(varclean %in% c("file_start", "n"))) {
        ...
        lambda_lin_pw <-
          ...
          ) # THESE ESTIMATES ARE NOW PRECISION WEIGHTED
...
for (var in names(rot_list)) {
  ...
  coefs_record[, paste0(var, "_pivot1")] <-
    summary(cox_mod_weighted)$coefficients[1, 1]
  # Assign Cox model, outputs
  assign(paste0("cox_mod_weighted", var), cox_mod_weighted)
  rm(coef, se)
...
print(vals)
write.csv(vals, "plots/precision_weighted/thesis-cox-coefs-precision-weighted-with-meas-err-adjustment.csv")
```


## Plots
```{r}
vals <- read.csv("plots/precision_weighted/thesis-cox-coefs-precision-weighted-with-meas-err-adjustment.csv")
...
for (i in 1:length(rot_list)) {
  ...
  ev_nums <- get_ev_nums(get(paste0("cox_mod_weighted", comp_name)))
...
write.csv(rec_dat, "plots/precision_weighted/rec_dat_meas_error_precision_weighted.csv") # This just saves so it's not a pain to get back if needed
```

Plot difference:
```{r}
rec_dat <- read.csv("plots/precision_weighted/rec_dat_meas_error_precision_weighted.csv")
rec_dat$X <- NULL
...
rec_dat$Model[rec_dat$Model == "measurement_adjusted"] <- "with within-person variation correction (precision-weighted)"
rec_dat <- do_plot_admin(rec_dat, mod_levels = c( "with within-person variation correction (precision-weighted)", "original"))
my_cols <- get_cols(2)
...
## save plot to file----------------------------------------------------------------------------------------------
svg(
  "plots/precision_weighted/thesis-multivariable-precision-weighted.svg",
  width = 10,
  height = 5
)
print(p)
