...
coefs_record <- data.frame(matrix(nrow = 1, ncol = 0))
...
for (var in names(rot_list)) {
  ...
  coefs_record[, paste0(var, "_pivot1")] <-
    summary(cox_mod_month_of_year)$coefficients[1, 1]
  assign(paste0("cox_mod_month", var), cox_mod)
  rm(coef, se)
...
df_left <- df_limit[order(match(df_limit$eid, df_rm$eid)), ] # I believe this was harmlessly wrong previously (harmless as both datasets are already ordered by eid)
...
# CHECK 
if (!isTRUE(all.equal(colnames(ests), colnames(coefs_record)))){
  stop("Something went wrong in column names")
}

# Write out bootstrap for reuse
saveRDS(ests, "bootstrap_ests_monthofyear_for_reuse.RDS")
```


```{r}
# Now reload data for calculations
ests <- readRDS("bootstrap_ests_monthofyear_for_reuse.RDS")
...
diffs <- ests-coefs_record[rep(1, nrow(ests)), colnames(ests)] # Subtract overall estimate from estimate in bootstrap sample 
...
print(vals)
write.csv(vals, "plots/month_of_year/thesis-cox-coefs-monthofyear-with-meas-err-adjustment.csv")
```

## Plots
```{r}
vals <- read.csv("plots/month_of_year/thesis-cox-coefs-monthofyear-with-meas-err-adjustment.csv")
...
for (i in 1:length(rot_list)) {
  ...
  ev_nums <- get_ev_nums(get(paste0("cox_mod_month", comp_name)))
...
write.csv(rec_dat, "plots/month_of_year/rec_dat_meas_error_month_of_year.csv") # This just saves so it's not a pain to get back if needed
```

Plot difference:
```{r}
rec_dat <- read.csv("plots/month_of_year/rec_dat_meas_error_month_of_year.csv")
rec_dat$X <- NULL
...
rec_dat$Model[rec_dat$Model == "measurement_adjusted"] <- "with within-person variation correction (month-of-year-adjusted)"
rec_dat <- do_plot_admin(rec_dat, mod_levels = c( "with within-person variation correction (month-of-year-adjusted)", "original"))
my_cols <- get_cols(2)
...
## save plot to file----------------------------------------------------------------------------------------------
svg(
  "plots/month_of_year/thesis-multivariable-month-of-year.svg",
  width = 10,
  height = 5
)
print(p)
...
# Why is season not that important? 
