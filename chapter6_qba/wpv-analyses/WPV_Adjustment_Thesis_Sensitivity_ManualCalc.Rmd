...
Why wouldn't you expect them to be exact? It's to do with the covariates - without covariates they seem to be identical to many d.p:
```{r}
# Regression calibration procedure
## Regression calibration in full dataset
for (var in names(rot_list)) {
  ...
  mat <- models$coefficients
  inverse_mat_relevant <- solve(mat[2:4,])
  coef_before <- summary(cox_basic)$coefficients[1:3, 1]
  changed_coef <- inverse_mat_relevant %*% coef_before
...
It's a little tricky to exactly get this to match because of the stratification on, rather than adjustment for, sex, and the treatment of age via age as the timescale. But if we remove stratification on sex and rather adjust for both we can show the identity: 
